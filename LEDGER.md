...
**Last updated: 2026-08-14.** Questions 1-4 answered (GLD
...
Lineage (only the last hop is verifiable here):
...
**Table superseded 2026-08-15** — two of its three rows
...
- **That anything runs.** No load has been attempted in this working
  ...
  `build/ReleaseSnowLeo*/` and nothing has been installed or loaded
...
- **The lineage above the `origin` remote.** Not checked against commit
  history.
...
- **Whether `AppleIntelGMA950GLDriver.bundle` exists on any given guest OS.**
  Doubtful beyond ~10.7-10.8 (GMA 950 Macs are 2006-era).
...
- **The capability ceiling of this route** — GMA950-class, GL 1.4/2.0-era.
  Unverified; note the shim's rendering half is GLRendererFloat
...
1. ~~How is the GLD loaded?~~ **Answered 2026-08-14.** `IOGLBundleName` is
   ...
   **Correction 2026-08-14 (later same day):** the Q1 write-up here first said
   ...
   GLD path. That was wrong twice over, and Q2's findings show why: (a) the
   cost *moves* (to the kernel-side contract), it does not
...
3. ~~What does `VMsvga2Surface.cpp` do that VMQemuVGA's surface client does
   ...
   **Reframed 2026-08-14:** VMQemuVGA's Phase A saw zero
   ...
   **Cost-estimate correction, 2026-08-14:** do **not** book the
