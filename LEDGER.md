...
| GLD contract size: **92 entry points** (`NUM_ENTRIES`), of which 48 are macro-generated generic forwarders (`GLD_DEFINE_GENERIC`); the hand-written ones read so far (`gldInitializeLibrary`, `gldGetVersion`, `gldGetRendererInfo`) also forward, with logging and a `#if 0` patch block | `GLD/EntryPointNames.h:32`, `EntryPointNames.c:33-38`, `VMsvga2GLDriver.c:40-46, 107-148` |
...
| Shim implements 75 of 92 entries: 48 `GLD_DEFINE_GENERIC` macro forwarders + 27 hand-written. **17 entries (idx 75-91, the 10.6.3 and 10.5.8 tails) have no function at all** — grep of `VMsvga2GLDriver.c` finds zero definitions | `GLD/VMsvga2GLDriver.c` (502 lines, read entire) |
| Shim behavior split: 1 active patcher (`gldGetString` overrides 0x1F00→"Zenith432", 0x1F01→"VMware SVGA II OpenGL Engine", 0x1F04→"VMsvga2GLDriver"); 2 disabled `#if 0` patchers (`gldGetRendererInfo`: flags\|0x4000, p[1]=0x17CD, p[17]=64, p[19]=64; `gldChoosePixelFormat`: flags\|0x4000, p[1]=0x501); 7 local stubs that never forward (CreateVertexArray/DestroyVertexArray→0, Create/DestroyComputeContext→−1, LoadHostBuffer/SyncBufferObject/SyncTexture→0); 20 typed forwarders; 48 generic forwarders | `GLD/VMsvga2GLDriver.c:107-502` |
...
2. ~~How large is the GLD contract?~~ **Answered 2026-08-14.** 92 named entry
   points in three eras + 2 init entries. Shim implements 75 (48 generic
   forwarders, 20 typed forwarders, 1 active patcher, 2 disabled patchers, 7
   local stubs); 17 tail entries unimplemented; rendering half routes to
